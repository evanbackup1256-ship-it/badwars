...
local drawingactor = loadstring(downloadFile('badscript/libraries/drawing.lua'), 'drawing')(...)
local redline = {Teams = {}}
...
local function warningRoutine(hash)
	local path = 'badscript/profiles/agreementhash.txt'
...
if not select(1, ...) then
	if run_on_actor then
		local oldreload = shared.Badreload
		Bad.Load = function()
		...
		task.spawn(function()
			repeat task.wait() until not shared.Bad
			local executionString = "loadfile('badscript/main.lua')("..drawingactor..")"
			...
			if oldreload then
				executionString = 'shared.Badreload = true\n'..executionString
			end
		...
		Bad.Load = function()
			notif('Bad', 'Missing actor functions.', 10, 'alert')
		end
	end
	...
	if game.PlaceId == 126691165749976 then
		...
		local function processPlayer(plr)
			if tonumber(plr.Name) then
				...
				Bad:Clean(plr:GetAttributeChangedSignal('team_id'):Connect(function()
					redline.Teams[plr.Name] = plr:GetAttribute('team_id')
					task.spawn(updatePlayer, plr)
				end))
			end
		...
		local function processMatch(match)
			if match and match.Name == 'Match' then
				Bad:Clean(match.DescendantAdded:Connect(processPlayer))
		...
		Bad:Clean(replicatedStorage.ReadOnly.ChildAdded:Connect(processMatch))
...
run(function()
	...
	for _, v in getloadedmodules() do
		if v:GetFullName() == 'Start.Client.ClientRoot' then
			...
			if not rawget(root, 'loaded') then
				repeat
					task.wait()
				until rawget(root, 'loaded') or Bad.Loaded == nil
			end
	...
	if not root then
		lplr:Kick('Failed to find root class, please contact usingINales on discord.')
		return
	end
	...
	if redline.ActionEventPacket then
		Bad:Clean(redline.ActionEventPacket.OnClientEvent:Connect(function(data)
	...
	Bad:Clean(BadEvents.MatchEnded.Event:Connect(function(won)
		if won then
			wins:Increment()
		end
	end))

	Bad:Clean(lplr.PlayerGui.ChildAdded:Connect(function(obj)
		if obj.Name == 'MatchResultsScreen' then
			...
			if obj then
				...
				BadEvents.MatchEnded:Fire(obj.Text:find('WON') and true or false, results)
			end
		end
	end))
end)
