...
AntiFall = Bad.Categories.Blatant:CreateModule({
	...
	Function = function(callback)
		if callback then
			if Method and Method.Value == 'Part' then
				local debounce = tick()
				...
				part.Material = Enum.Material[Material and Material.Value or 'Plastic']
				...
				part.CanCollide = Mode and Mode.Value == 'Collide'
				part.Anchored = true
				part.CanQuery = false
				...
				AntiFall:Clean(part.Touched:Connect(function(touchedpart)
					if touchedpart and touchedpart.Parent == lplr and lplr.Character and entitylib.isAlive and debounce < tick() then
				...
				repeat
					if entitylib.isAlive and entitylib.character and entitylib.character.RootPart then
						...
						rayCheck.FilterDescendantsInstances = {gameCamera, lplr and lplr.Character, part}
					...
					task.wait(0.1)
				until not AntiFall or not AntiFall.Enabled
			else
				local lastpos
