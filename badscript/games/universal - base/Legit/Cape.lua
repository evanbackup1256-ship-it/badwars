...
local function createMotor(char)
	...
	motor.Part0 = part
	motor.Part1 = char.Character:FindFirstChild('UpperTorso') or (char.RootPart and char.RootPart)
...
Cape = Bad.Legit:CreateModule({
	...
	Function = function(callback)
		if callback then
			...
			if Texture and Texture.Value and Texture.Value:find('.webm') then
				local decal = Instance.new('VideoFrame')
				decal.Video = getcustomasset and getcustomasset(Texture.Value) or Texture.Value
				...
				local texVal = Texture and Texture.Value or ''
				decal.Image = texVal ~= '' and (texVal:find('rbxasset') and texVal or (assetfunction and assetfunction(texVal))) or 'rbxassetid://14637958134'
				decal.Size = UDim2.fromScale(1, 1)
				decal.BackgroundTransparency = 1
				decal.Parent = capesurface
			end
			...
			repeat
				if motor and entitylib.isAlive and entitylib.character and entitylib.character.RootPart then
				...
				task.wait()
			until not Cape or not Cape.Enabled
		else
			part = nil
			motor = nil
		end
