...
local function isTarget(plr)
	return (table.find(Bad.Categories.Targets.ListEnabled, plr.Name) or tempTargets[plr.Name]) and true
end
...
run(function()
	function CheatFlags:Flag(plr, flagtype, limit)
		...
		if flags[flagtype] > limit then
			...
			BadEvents.CheatFlagged:Fire(plr, flagtype)
		end
	end
...
run(function()
	...
	if not (pl.Bullet and pl.SwitchTable) then
		repeat
			getShootFunction()
			task.wait()
		until pl.Bullet and pl.SwitchTable or Bad.Loaded == nil
	...
	Bad:Clean(replicatedStorage.Killfeed.ChildAdded:Connect(function(obj)
		local names = {}
		...
		BadEvents.PlayerKill:Fire(unpack(names))
	...
	Bad:Clean(BadEvents.Arrested.Event:Connect(function()
		arrests:Increment()
	end))

	Bad:Clean(replicatedStorage.Remotes.MessageReceived.OnClientEvent:Connect(function(msg)
		if msg:find('kicked') then
			cheaterkicked:Increment()
			BadEvents.CheaterKicked:Fire(msg:sub(1, msg:find(' ')))
		end
	end))

	Bad:Clean(entitylib.Events.EntityUpdated:Connect(function(ent)
		if ent.Player and ent.Player.Team == teams.Inmates then
			Bad.Categories.Friends.ColorUpdate:Fire()
		end
	end))
	...
	for _, v in {'EntityAdded', 'LocalAdded'} do
		Bad:Clean(entitylib.Events[v]:Connect(function()
			OriginScanner:UpdateIgnore()
		end))
	end

	Bad:Clean(runService.RenderStepped:Connect(function()
