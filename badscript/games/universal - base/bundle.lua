...
local getcustomasset = Bad.Libraries.getcustomasset
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('badscript/assets/new/blur.png')
...
local function isTarget(plr)
	return table.find(Bad.Categories.Targets.ListEnabled, plr.Name) and true
end
...
local function canClick()
	...
	return (not Bad.gui.ScaledGui.ClickGui.Visible) and (not inputService:GetFocusedTextBox())
end
...
local function serverHop(pointer, filter)
	visited = shared.Badserverhoplist and shared.Badserverhoplist:split('/') or {}
	if not table.find(visited, game.JobId) then
		table.insert(visited, game.JobId)
	end
	if not pointer then
		notif('Bad', 'Searching for an available server.', 2)
	end
...
Bad:Clean(lplr.OnTeleport:Connect(function()
	if not tpSwitch then
		tpSwitch = true
		queue_on_teleport("shared.Badserverhoplist = '"..table.concat(visited, '/').."'\nshared.Badserverhopprevious = '"..game.JobId.."'")
	end
end))
...
Bad.Libraries.entity = entitylib
Bad.Libraries.whitelist = whitelist
Bad.Libraries.prediction = prediction
Bad.Libraries.hash = hash
Bad.Libraries.auraanims = {
	Normal = {
...
run(function()
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				olduninject = Bad.Uninject
				Bad.Uninject = function()
					notif('Bad', 'No escaping the private members :)', 10)
				end
			end
		end
	end
	...
	function whitelist:announce(text)
		...
		if success then
			return sendToast({
				toastTitle = text,
				iconImage = getcustomasset('badscript/assets/new/Bad.png'),
				swipeUpDismiss = true,
		...
		icon.Image = getcustomasset('badscript/assets/new/Bad.png')
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('badscript/profiles/whitelist.json') and readfile('badscript/profiles/whitelist.json') or nil
			end
			...
			if not whitelist.connection then
				...
				Bad:Clean(whitelist.connection)
			end
			...
			if entitylib.Running and Bad.Loaded then
				entitylib.refresh()
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('badscript/profiles/whitelist.json', whitelist.textdata)
				end)
			end

			if whitelist.data.KillBad then
				Bad:Uninject()
				return true
			end
...
Jesus = Bad.Categories.Blatant:CreateModule({
	Name = 'Jesus',
	Function = function(callback)
		if callback then
			local terrain = workspace:FindFirstChildWhichIsA('Terrain')
...
MouseTP = Bad.Categories.Blatant:CreateModule({
	Name = 'MouseTP',
	Function = function(callback)
		if callback then
			local position
...
run(function()
	...
	SilentAim = Bad.Categories.Combat:CreateModule({
		Name = 'SilentAim',
		Function = function(callback)
			if CircleObject then
...
TimeChanger = Bad.Legit:CreateModule({
	Name = 'Time Changer',
	Function = function(callback)
		if callback then
...
MurderMystery = Bad.Categories.Minigames:CreateModule({
	Name = 'MurderMystery',
	Function = function(callback)
		if callback then
...
Arrows = Bad.Categories.Render:CreateModule({
	Name = 'Arrows',
	Function = function(callback)
		if callback then
			...
			Arrows:Clean(Bad.Categories.Friends.ColorUpdate.Event:Connect(function()
...
Chams = Bad.Categories.Render:CreateModule({
	Name = 'Chams',
	Function = function(callback)
		if callback then
			...
			Chams:Clean(Bad.Categories.Friends.ColorUpdate.Event:Connect(function()
...
ESP = Bad.Categories.Render:CreateModule({
	Name = 'ESP',
	Function = function(callback)
		if callback then
			...
			if ColorFunc[methodused] then
				ESP:Clean(Bad.Categories.Friends.ColorUpdate.Event:Connect(function()
...
Fullbright = Bad.Categories.Render:CreateModule({
	Name = 'Fullbright',
	Function = function(callback)
		if callback then
...
GamingChair = Bad.Categories.Render:CreateModule({
	...
	Function = function(callback)
		if callback then
			...
			--movingsound.SoundId = downloadBadAsset('Bad/assets/ChairRolling.mp3')
			movingsound.Volume = 0.4
			movingsound.Looped = true
			movingsound.Parent = workspace
			flyingsound = Instance.new('Sound')
			--flyingsound.SoundId = downloadBadAsset('Bad/assets/ChairFlying.mp3')
...
NameTags = Bad.Categories.Render:CreateModule({
	Name = 'NameTags',
	Function = function(callback)
		if callback then
			...
			if ColorFunc[methodused] then
				NameTags:Clean(Bad.Categories.Friends.ColorUpdate.Event:Connect(function()
					ColorFunc[methodused](Color.Hue, Color.Sat, Color.Value)
				end))
			end
...
PlayerModel = Bad.Categories.Render:CreateModule({
	Name = 'PlayerModel',
	Function = function(callback)
		if callback then 
...
Radar = Bad:CreateOverlay({
	Name = 'Radar',
	Icon = getcustomasset('badscript/assets/new/radaricon.png'),
	...
	Function = function(callback)
		if callback then
			...
			Radar:Clean(Bad.Categories.Friends.ColorUpdate.Event:Connect(function()
...
Search = Bad.Categories.Render:CreateModule({
	Name = 'Search',
	Function = function(callback)
		if callback then
		...
		if callback then
			...
			SessionInfo:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
				if not teleportedServers then
					...
					queue_on_teleport("shared.Badsessioninfo = '"..httpService:JSONEncode(Bad.Libraries.sessioninfo.Objects).."'")
				end
			end))

			if shared.Badsessioninfo then
				for i, v in httpService:JSONDecode(shared.Badsessioninfo) do
					if Bad.Libraries.sessioninfo.Objects[i] and v.Saved then
						Bad.Libraries.sessioninfo.Objects[i].Value = v.Value
					end
				end
			end
...
Bad:Clean(SessionInfo.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
	if Bad.ThreadFix then
		setthreadidentity(8)
	end
	local newside = SessionInfo.Children.AbsolutePosition.X > (Bad.gui.AbsoluteSize.X / 2)
...
Bad.Libraries.sessioninfo:AddItem('Time Played', os.clock(), function(value)
...
Tracers = Bad.Categories.Render:CreateModule({
	Name = 'Tracers',
	Function = function(callback)
		if callback then
			...
			Tracers:Clean(Bad.Categories.Friends.ColorUpdate.Event:Connect(function()
				ColorFunc(Color.Hue, Color.Sat, Color.Value)
			end))
...
Gravity = Bad.Categories.World:CreateModule({
	Name = 'Gravity',
	Function = function(callback)
		if callback then
...
Parkour = Bad.Categories.World:CreateModule({
	Name = 'Parkour',
	Function = function(callback)
		if callback then 
			local oldfloor
...
Xray = Bad.Categories.World:CreateModule({
	Name = 'Xray',
	Function = function(callback)
		if callback then
