...
lplr:Kick('Bedwars is no longer supported by Bad V4, thank you for 5 years of support ❤️')

local Bad = shared.Bad
local entitylib = Bad.Libraries.entity
local targetinfo = Bad.Libraries.targetinfo
local sessioninfo = Bad.Libraries.sessioninfo
local uipallet = Bad.Libraries.uipallet
local tween = Bad.Libraries.tween
local color = Bad.Libraries.color
local whitelist = Bad.Libraries.whitelist
local prediction = Bad.Libraries.prediction
...
local function hotbarSwitch(slot)
	if slot and store.inventory.hotbarSlot ~= slot then
		...
		BadEvents.InventoryChanged.Event:Wait()
		return true
	end
	return false
end
			...
			if hum and humrootpart then
				...
				if plr == lplr then
					...
					table.insert(entitylib.Connections, char.AttributeChanged:Connect(function(attr)
						BadEvents.AttributeChanged:Fire(attr)
					end))
				else
...
run(function()
	...
	for i, v in remoteNames do
		...
		if remote == '' then
			notif('Bad', 'Failed to grab remote ('..i..')', 10, 'alert')
		end
		remotes[i] = remote
	end
	...
	local function updateStore(new, old)
		...
		if new.Inventory ~= old.Inventory then
			...
			if newinv.inventory.items ~= oldinv.inventory.items then
				BadEvents.InventoryAmountChanged:Fire()
	...
	for _, event in {'MatchEndEvent', 'EntityDeathEvent', 'BedwarsBedBreak', 'BalloonPopped', 'AngelProgress', 'GrapplingHookFunctions'} do
		if not Bad.Connections then return end
		bedwars.Client:WaitFor(event):andThen(function(connection)
			Bad:Clean(connection:Connect(function(...)
				BadEvents[event]:Fire(...)
			end))
		end)
	end

	Bad:Clean(bedwars.ZapNetworking.EntityDamageEventZap.On(function(...)
		BadEvents.EntityDamageEvent:Fire({
			entityInstance = ...,
			damage = select(2, ...),
			damageType = select(3, ...),
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		Bad:Clean(bedwars.ZapNetworking[event..'Zap'].On(function(...)
			local data = {
				blockRef = {
					blockPosition = ...,
				},
	...
	task.spawn(function()
		pcall(function()
			repeat task.wait() until store.matchState ~= 0 or Bad.Loaded == nil
			if Bad.Loaded == nil then return end
			mapname = workspace:WaitForChild('Map', 5):WaitForChild('Worlds', 5):GetChildren()[1].Name
			mapname = string.gsub(string.split(mapname, '_')[2] or mapname, '-', '') or 'Blank'
		end)
	end)

	Bad:Clean(BadEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
		if bedTable.player and bedTable.player.UserId == lplr.UserId then
			beds:Increment()
		end
	end))

	Bad:Clean(BadEvents.MatchEndEvent.Event:Connect(function(winTable)
		if (bedwars.Store:getState().Game.myTeam or {}).id == winTable.winningTeamId or lplr.Neutral then
			wins:Increment()
		end
	end))

	Bad:Clean(BadEvents.EntityDeathEvent.Event:Connect(function(deathTable)
	...
	pcall(function()
		if getthreadidentity and setthreadidentity then
			...
			task.spawn(function()
				repeat
					task.wait(0.1)
				until Bad.Loaded == nil or bedwars.AppController:isAppOpen('BedwarsItemShopApp')
