...
local function CreateWindow(self)
	...
	window.Parent = Bad.gui.ScaledGui
	...
	close.MouseButton1Click:Connect(function()
		window.Visible = false
		Bad.gui.ScaledGui.ClickGui.Visible = true
	end)
	...
	windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
		...
		children.CanvasSize = UDim2.fromOffset(0, windowlist.AbsoluteContentSize.Y / Bad.guiscale.Scale)
	end)
	table.insert(Bad.Windows, window)
...
Bad.Components.HotbarList = function(optionsettings, children, api)
	if Bad.ThreadFix then
		setthreadidentity(8)
	end
	local optionapi = {
	...
	textbuttonicon.Image = getcustomasset('badscript/assets/new/add.png')
	...
	windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
		...
		hotbarlist.Size = UDim2.fromOffset(220, math.min(43 + windowlist.AbsoluteContentSize.Y / Bad.guiscale.Scale, 603))
	end)
	...
	function optionapi:AddHotbar(data)
		...
		hotbar.MouseButton1Click:Connect(function()
			...
			if ind == optionapi.Selected then
				Bad.gui.ScaledGui.ClickGui.Visible = false
...
AutoHotbar = Bad.Categories.Inventory:CreateModule({
	...
	Function = function(callback)
		if callback then
			...
			AutoHotbar:Clean(BadEvents.InventoryAmountChanged.Event:Connect(sortCallback))
		end
	end,
