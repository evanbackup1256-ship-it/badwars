local loadstring = function(...)
	...
	if err and Bad then Bad:CreateNotification('Bad', 'Failed to load : '..err, 30, 'alert') end
	return res
end
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/evanbackup1256-ship-it/badwars/'..readfile('badscript/profiles/commit.txt')..'/'..select(1, path:gsub('badscript/', '')), true) end)
...
local vm = loadstring(downloadFile('badscript/libraries/vm.lua'), 'vm')()
...
run(function()
	...
	if not jb.VehicleController.toggleLocalLocked or not jb.VehicleController.NitroShopVisible then
		repeat task.wait() until (jb.VehicleController.toggleLocalLocked and jb.VehicleController.NitroShopVisible) or Bad.Loaded == nil
	...
	function jb:FireServer(id, ...)
		if not remotes[id] then
			notif('Bad', 'Failed to find remote ('..id..')', 10, 'alert')
			return
		end
