...
local EventHandlerUtils = assert(dependencies.EventHandlerUtils, "EventHandlerUtils missing")

local function getMemoryCategory(): string
    local debugger = debug
    if type(debugger) == "table" and type(debugger.getmemorycategory) == "function" then
        local ok, value = pcall(debugger.getmemorycategory)
...
function Connection.new<T...>(signal: Signal<T...>, fn: SignalHandler<T...>): Connection<T...>
	return setmetatable({
		...
		_memoryCategory = getMemoryCategory(),
		_signal = signal,
		_fn = fn,
	}, Connection) :: any
end
