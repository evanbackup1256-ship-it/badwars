...
Bad.Place = 16483433878
if isfile('badscript/games/'..Bad.Place..'.lua') then
	loadstring(readfile('badscript/games/'..Bad.Place..'.lua'), 'blocktales')()
else
	if not shared.BadDeveloper then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/evanbackup1256-ship-it/badwars/'..readfile('badscript/profiles/commit.txt')..'/games/'..Bad.Place..'.lua', true)
		end)
		if suc and res ~= '404: Not Found' then
			loadstring(downloadFile('badscript/games/'..Bad.Place..'.lua'), 'blocktales')()
		end
	end
end
