...
local BaseObject = assert(dependencies.BaseObject, "BaseObject missing")
local ServiceInitLogger = assert(dependencies.ServiceInitLogger, "ServiceInitLogger missing")
local Signal = assert(dependencies.Signal, "Signal missing")
	...
	while next(self._serviceTypesToStartSet) do
		...
		if service.Start then
			...
			task.spawn(function()
				...
				if debug and type(debug.setmemorycategory) == "function" then
				pcall(debug.setmemorycategory, serviceName)
			end
			...
			if not isDead then
				error(debug and debug.traceback and debug.traceback(current, string.format("Starting service %q yielded", serviceName)) or string.format("Starting service %q yielded", serviceName))
			end
		end
	end
...
function ServiceBag._initService(self: ServiceBag, serviceType)
	...
	if service.Init then
		...
		local isDead = coroutine.status(current) == "dead"
		if not isDead then
			error(debug and debug.traceback and debug.traceback(current, string.format("Initializing service %q yielded", serviceName)) or string.format("Initializing service %q yielded", serviceName))
		end
	end
